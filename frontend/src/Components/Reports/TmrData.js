...
function TmrData({timecards, actions}) {
  ...
  const dataColumns = [
    ...
    },
    {
      ...
      align: 'center',
      render:  (record) => {
        let task = tmrTasks.find(x=>x.TaskId === record.TaskTypeId || x.TaskId === record.TaskId);
        return (
          <Space direction="vertical">
                {task.Name}
            </Space>
...
//Get all TMR Tasks
let tmrAvailableTasks = timecards.map(x=>x.AvailableTasks).flat().filter(x=>x.Type === 'TMR');
...
let tmrTasks =  allTasks.map((task) => {
  let taskId = task.TaskTypeId ? task.TaskTypeId : task.TaskId;
...
for(var i = 0; i < tmrTasks.length; i++){
  ...
  let notesdata = tmrTasks[i].Notes.map((note)=> { return {...note, ...taskData}});
  console.log('!!! NOTES', notesdata);
  TMRData.push(...notesdata);
}

setTMRData(TMRData);
console.log('!!! Tasks', TMRData);
...
const OnTMRTypeChanged = async(type) => {
  ...
  return (
    <div>
      <Row>
      <Col> 
      <Select  style={{ width: '200px' }} onChange={(type) => setSelectedTmrType(type)} >
        {tmrTasks.map((task) => (
     ...
     <Table
                ...
                bordered
                dataSource={selectedTmrType ? 
                  tmrData.filter(x=>x.TaskId === selectedTmrType || x.TaskTypeId === selectedTmrType).sort((a,b) => moment(b.StartTime) - moment(a.StartTime)) 
                  : tmrData.sort((a,b) => moment(b.StartTime) - moment(a.StartTime))}
