...
function ToDoTask({ actions, tasks, todoData }) {
  ...
  useEffect(() => {
    ...
    setData(todoData.filter((x) => x.Status !== "Completed" && x.TaskType === taskType).sort((a, b) => moment(a.ReviewDate) - moment(b.ReviewDate)));
  }, [todoData]);
    ...
    try {
      ...
      if (index > -1) {
        ...
        console.log(
          "Setting New Data",
          newData,
          newData.filter((x) => x.TaskType === taskType)
        );
        setData(newData.filter((x) => x.TaskType === taskType).sort((a, b) => moment(a.ReviewDate) - moment(b.ReviewDate)));
      }
    } catch (errInfo) {
      console.log("Validate Failed:", errInfo);
    }
